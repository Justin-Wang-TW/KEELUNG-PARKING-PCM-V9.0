...
const ChecklistDashboard: React.FC<ChecklistDashboardProps> = ({
  ...
  const filteredSubmissions = submissions.filter(sub => {
    ...
    if (sub.stationCode === StationCode.CONTRACT_TEAM && currentUser.role === UserRole.OPERATOR) return false;
  ...
  const handleExport = () => {
    ...
    filteredSubmissions.forEach(sub => {
      sub.results.forEach(res => {
        ...
        dataToExport.push({
          '檢核月份': sub.yearMonth,
          '場站': STATIONS.find(s => s.code === sub.stationCode)?.name || sub.stationName || sub.stationCode,
          '填表人': sub.submittedBy,
