...
const TaskList: React.FC<TaskListProps> = ({ tasks = [], currentUser, onEditTask, onModifyTask, onViewDetail }) => {
  ...
  const filteredTasks = useMemo(() => {
    ...
    return tasks.filter((task) => {
      ...
      // Hide CONTRACT_TEAM tasks from OPERATOR
      if (task.stationCode === StationCode.CONTRACT_TEAM && currentUser.role === UserRole.OPERATOR) {
        return false;
      }
  ...
  const handleExport = () => {
    const dataToExport = filteredTasks.map(task => ({
      '場站': STATIONS.find(s => s.code === task.stationCode)?.name || task.stationName || task.stationCode,
      '工項名稱': task.itemName,
