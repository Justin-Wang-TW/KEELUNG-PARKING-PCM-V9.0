...
const CreateTaskModal: React.FC<CreateTaskModalProps> = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isCommon) {
      targetStations = STATIONS.filter(s => s.code !== StationCode.CONTRACT_TEAM).map(s => s.code);
    } else {
                 ...
                 <input
                   ...
                   onChange={(e) => {
                    ...
                    if (checked) {
                      ...
                      const commonStations = STATIONS.filter(s => s.code !== StationCode.CONTRACT_TEAM).map(s => s.code);
