...
const AssetManagement: React.FC<AssetManagementProps> = ({ currentUser, assets, checkRecords, checkBatches = [], onDelete, onSubmitCheck, onSaveBatch, onSaveBatchAssets, onEdit }) => {
  ...
  const availableStations = useMemo(() => {
    ...
    if (currentUser.role === UserRole.OPERATOR) {
      stations = STATIONS.filter(s => s.code !== StationCode.CONTRACT_TEAM);
    }
  ...
  const filteredAssets = useMemo(() => {
    return assets.filter(asset => {
      ...
      if (asset.stationCode === StationCode.CONTRACT_TEAM && currentUser?.role === UserRole.OPERATOR) {
        return false;
      }
  ...
  const filteredRecords = useMemo(() => {
    return checkRecords.filter(record => {
      ...
      if (record.stationCode === StationCode.CONTRACT_TEAM && currentUser?.role === UserRole.OPERATOR) {
        return false;
      }
  ...
  const filteredBatches = useMemo(() => {
    return checkBatches.filter(batch => {
      ...
      if (batch.stationCode === StationCode.CONTRACT_TEAM && currentUser?.role === UserRole.OPERATOR) {
        return false;
      }
  ...
  const handleExportAssets = () => {
    const dataToExport = filteredAssets.map(asset => ({
      '場站名稱': STATIONS.find(s => s.code === asset.stationCode)?.name || asset.stationName,
      '財產名稱': asset.name,
      '數量': asset.quantity,
      '單位': asset.unit || '-',
      '設備商': asset.supplier || '-',
      ...
      filteredBatches.forEach(batch => {
        batch.results.forEach(res => {
          dataToExport.push({
            '年度': batch.year,
            '期別': batch.period,
            '場站': STATIONS.find(s => s.code === batch.stationCode)?.name || batch.stationName,
