...
const MeetingRecords: React.FC<MeetingRecordsProps> = ({ meetings = [], currentUser, onSave }) => {
  ...
  const filteredMeetings = (meetings || []).filter(m => {
    ...
    // Hide CONTRACT_TEAM meetings from OPERATOR
    if (m.stationCode === StationCode.CONTRACT_TEAM && currentUser.role === UserRole.OPERATOR) {
        return false;
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-lg animate-fade-in">
             ...
             <form onSubmit={handleSubmit} className="p-6 space-y-4">
               <div>
                 <label className="block text-sm font-bold text-gray-700 mb-1">關聯場站</label>
                 <select
                   value={stationCode}
